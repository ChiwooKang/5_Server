...
public class ServletEx2 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// radio 버튼은 하나만 선택 가능 == 값 1개만 서버로 제출됨
		String type = req.getParameter("type");
		
		// select 박슨 1개의 option만 선택 가능 == 값 1개만 서버로 제출됨
		String coffe = req.getParameter("coffe");
		
		// checkox와 같이 하나의 name 속성으로 여러값이 전달 될 경우
		...
		// (HTML, CSS, JS등 브라우저가 해설할 수 있는 코드는 모두 해석되어 보여짐)
		...
		if (opt != null) {
			 ...
			 * 
			 * - 요청에 따라서 응답되는 화면 (HTML)을 실시간으로 만들어내서 클라이언트에게 응답하는 프로젝트
			 * 
			 */
